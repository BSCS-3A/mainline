<?php 
    ...
    if($numrows > 0){
      while( $row = mysqli_fetch_assoc($result)){
        if(!empty($row['photo'])){  
            $string .= "<tr candid ='".$row['candidate_id']."'><td style='text-align:middle;'><input type='file' name='image' class='image' style='display:none;'><button type='button' onclick='changePhoto(this)'>Change Photo</button></td>";
        }else{
            $string .= "<tr candid ='".$row['candidate_id']."'><td style='text-align:middle;'><input type='file' name='image' class='image' style='display:none;'><button type='button' onclick='changePhoto(this)'>Upload Photo</button></td>";
        }
