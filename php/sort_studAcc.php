...
while($row = mysqli_fetch_array($result))
{
 ...
 $sub_array[] = cleanOutput($conn,$row["lname"]);
 $sub_array[] = cleanOutput($conn,$row["fname"]);
 $sub_array[] = cleanOutput($conn,$row["Mname"]);
 $sub_array[] = $row["gender"];
 $sub_array[] = cleanOutput($conn,$row["bumail"]);
 $sub_array[] = $row["grade_level"];
 $sub_array[] = $row["otp"];
 if(isset($rowx) && $now >= $rowx[1] && $now <= $rowx[2]){
